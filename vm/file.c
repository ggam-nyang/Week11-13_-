...
file_backed_swap_out (struct page *page) {
	...
	// spt_remove_page (curr->pml4, page);
	page->frame = NULL;
		...
		if (mfi->start == (uint64_t) addr) {
			...
			{				
				...
				if (pml4_is_dirty (curr->pml4, p->va)) {
					...
					// file_write_at (fp->file, p->va, fp->size, fp->ofs);
...
static bool lazy_load_file (struct page *page, void *aux)
{
	...
	free(map_info);
	return true;
	// if (map_info->read_bytes > 0) {
	// 	file_seek(map_info->file, map_info->offset);
	// 	if (file_read (map_info->file, page->va, map_info->read_bytes) != (off_t) map_info->read_bytes) {
	// 		vm_dealloc_page (page);
	// 		free (map_info);
	// 		return false;
	// 	}
	// }
	// memset (page->va + map_info->read_bytes, 0, PGSIZE - map_info->read_bytes);
